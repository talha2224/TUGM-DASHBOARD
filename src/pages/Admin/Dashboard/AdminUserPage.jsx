                    ...
                    <tbody>
                        {filteredData.map(user => (
                            <tr key={user._id}>
                                ...
                                <td className="py-2 px-4 border-b">
                                    {user.sellerMode ? 'Enabled' : 'Disabled'}
                                </td>
                                <td className="py-2 px-4 border-b">{user.coins}</td>
                                <td className="py-2 px-4 border-b">{user.followers}</td>
                                <td className="py-2 mt-3 px-4 border-b space-x-2 flex flex-wrap">
                                    <button
                                        className="px-3 py-1 bg-red-100 text-red-700 text-xs rounded-md text-nowrap truncate mt-1"
                                        onClick={() => handleBlockUnblock(user._id, !user.isBlocked)}
                                    >
                                        {user.isBlocked ? 'Unblock' : 'Block'}
                                    </button>
                                    <button
                                        className="px-3 py-1 bg-blue-100 text-blue-700 text-xs rounded-md text-nowrap truncate mt-1"
                                        onClick={() => handleToggleSellerMode(user._id, user.sellerMode)}
                                    >
                                        Toggle Seller
                                    </button>
                                    <button onClick={() => {setSelectedUserId(user._id);setShowModal(true);}} className="bg-[#F2F2F2] rounded-md px-3 py-1 text-xs">Send Notification</button>
